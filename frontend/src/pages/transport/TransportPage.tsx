...
const rideOptions: RideOption[] = [
  { id: "bike", name: "SMAJ Bike", note: "Quick and affordable", eta: "2 min", seats: 1, pricePerKm: 1.8, icon: "bike" },
  { id: "economy", name: "Economy", note: "Everyday rides", eta: "4 min", seats: 4, pricePerKm: 3.6, icon: "car", popular: true },
  { id: "comfort", name: "Comfort", note: "Newer, spacious cars", eta: "6 min", seats: 4, pricePerKm: 5.2, icon: "car" },
  { id: "delivery", name: "Delivery", note: "Send packages safely", eta: "8 min", seats: 0, pricePerKm: 4.4, icon: "van" },
];
...
const RideIcon = ({ type }: { type: RideOption["icon"] }) => type === "bike"
...
const TransportPage = () => {
    ...
    try {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(async (pos) => {
          ...
          setNearbyDrivers(data.drivers.map((d) => ({
            driverId: d.driverId,
  ...
  const home = <div className="transport-home">
    ...
    <section className="transport-map-section"><MapCanvas pickup={pickup} destination={destination} drivers={nearbyDrivers} /></section>
  ...
  const chooseRide = <section className="transport-choose-layout">
    <div className="transport-ride-panel">
      ...
      <div className="transport-ride-list">{rideOptions.map((ride) => <button type="button" key={ride.id} className={selectedRide === ride.id ? "selected" : ""} onClick={() => setSelectedRide(ride.id)}>
        ...
        <strong>π {(ride.pricePerKm * Math.max(1, Math.min(2.4, destination.trim().length / 12))).toFixed(2)}</strong>
      </button>)}</div>
  ...
  const trackingPage = <section className="transport-tracking">
    ...
    <aside><div className="tracking-handle" /><span className="transport-eyebrow">DRIVER IS ON THE WAY</span><h1>Meet driver in 3 min</h1><div className="driver-profile"><span>MK</span><div><b>Assigned Driver</b><small><StarRoundedIcon /> 4.96 · Active</small></div><strong>{activeRide.name}<br /><small>{activeRide.seats ? `${activeRide.seats} seats` : "Package"}</small></strong></div><div className="tracking-progress"><span className="active" /><span /><span /></div><div className="tracking-details"><p><LocationOnRoundedIcon /><span><small>PICKUP</small><b>{activeTrip?.pickup || pickup}</b></span></p><p><PaymentsRoundedIcon /><span><small>FARE</small><b>π {(activeTrip?.price || quote).toFixed(2)} · Pi Wallet</b></span></p></div><div className="tracking-actions"><button>Contact driver</button><button>Safety</button></div><button className="transport-main-button" onClick={completeTrip}>Complete trip</button></aside>
  </section>;
...
const MapCanvas = ({ pickup, destination, tracking = false, drivers = [] }: { pickup: string; destination: string; tracking?: boolean; drivers?: Array<{ driverId: string; displayName: string; vehicleName: string; vehiclePlate: string; rating: number; isOnline: boolean; distanceKm?: number }> }) => (
  <div className={`transport-map ${tracking ? "is-tracking" : ""}`} aria-label="Route preview map">
    ...
    {drivers.length > 0 && !tracking ? <div className="driver-marker" style={{ left: "60%", top: "55%" }}><DirectionsCarRoundedIcon /><i>{drivers[0]?.displayName || "Driver"}</i></div> : null}
    <button type="button" className="map-location-button" aria-label="Use my current location"><MyLocationRoundedIcon /></button>
    <div className="map-status"><span /> Live driver network <b>{drivers.length || 128} nearby</b></div>
  </div>
);
