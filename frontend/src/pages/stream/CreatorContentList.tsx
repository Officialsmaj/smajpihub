...
const CreatorContentList = () => {
  ...
  return <div className="sw-table"><header><b>Video</b><b>Processing</b><b>Moderation</b><b>Visibility</b></header>{videos.map((video) => <div key={video._id}><span>{video.thumbnailUrl ? <img className="sw-content-thumb" src={video.thumbnailUrl} alt="" /> : <i className="purple"/>}<b>{video.title}</b></span><em>{video.processingStatus}</em><span>{video.moderationStatus}</span><span>{video.visibility}</span></div>)}</div>;
};
