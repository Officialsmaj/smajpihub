...
const BookingPage = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!providerId || !serviceId || !service) return;

    setSubmitting(true);
    setError(null);
    try {
      await createHealthBooking({
        providerId,
        serviceId,
        date: form.date,
        time: form.time,
          ...
          <div className="health-booking-layout">
            <form className="health-booking-form" onSubmit={handleSubmit}>
              ...
              <button className="health-booking-submit" type="submit" disabled={submitting}>
                {submitting ? "Booking..." : `Pay π ${total.toFixed(2)} and confirm`}
              </button>
            </form>
            <aside className="health-booking-summary-card">
              <h3>Appointment summary</h3>
              <p><strong>{provider.name}</strong></p>
              <p>{service.name} · {service.duration}</p>
