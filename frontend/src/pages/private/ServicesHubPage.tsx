...
const ServicesHubPage = () => {
  ...
  return <main className="private-page services-launcher">
    <div className="services-desktop-view">
      ...
      <section className="services-hub-grid">{visible.map((service) => isInProgress(service.slug) ? <article className="service-in-progress-card" key={service.slug}><ServiceArt index={service.atlasIndex} /><span className="service-status in-progress">IN PROGRESS</span><strong>{service.name}</strong><p>{service.description}</p></article> : <Link key={service.slug} to={servicePath(service.slug, service.live)} state={service.slug === "store" ? { showStoreEntrance: true } : undefined}><ServiceArt index={service.atlasIndex} /><span className={`service-status ${service.live ? "live" : "soon"}`}>{service.live ? "LIVE" : "Coming Soon"}</span><strong>{service.name}</strong><p>{service.description}</p><i><ArrowForwardOutlinedIcon /></i></Link>)}</section>
    ...
    <section className="services-mobile-view">
      ...
      <div className="mobile-services-grid">{orderedServices.map((service) => isInProgress(service.slug) ? <article className="service-in-progress-card" key={service.slug}><ServiceArt index={service.atlasIndex} /><em className="service-in-progress-badge">IN PROGRESS</em><strong>{service.name.replace("SMAJ ", "")}</strong><span>{serviceHints[service.slug]}</span></article> : <Link key={service.slug} to={servicePath(service.slug, service.live)} state={service.slug === "store" ? { showStoreEntrance: true } : undefined}><ServiceArt index={service.atlasIndex} />{service.live ? <em className="live-card-badge">LIVE</em> : null}<strong>{service.name.replace("SMAJ ", "")}</strong><span>{serviceHints[service.slug]}</span></Link>)}</div>
    </section>
  </main>;
};
