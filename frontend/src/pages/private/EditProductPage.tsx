...
const EditProductPage = () => {
  ...
  return <main className="private-page"><Link className="private-back-link" to="/seller">Back to Seller Dashboard</Link><section className="private-page-head"><div><p className="private-kicker">PRODUCT MANAGEMENT</p><h1>Edit Product</h1><p>Update listing details, price, image, and contact information.</p></div></section>
    <form className="private-form" onSubmit={(event) => void submit(event)}>
      ...
      <div className="private-form-row"><label>USDT price<input required type="number" min="0.01" step="0.01" value={form.priceUsdt} onChange={(event) => setForm({ ...form, priceUsdt: event.target.value, pricePi: formatPiInputValue(Number(event.target.value) / PI_USDT_RATE) })} /></label><label>Pi price<input required type="number" min="0.0000000001" step="any" value={form.pricePi} onChange={(event) => setForm({ ...form, pricePi: event.target.value, priceUsdt: String(Number(event.target.value) * PI_USDT_RATE) })} /></label></div>
      ...
      {form.variants.length ? <div className="product-variant-list">{form.variants.map((variant, index) => <article key={index}><div className="product-variant-head"><strong>Variant {index + 1}</strong><button type="button" onClick={() => removeVariant(index)}>Remove</button></div><div className="private-form-row">{variantFields.map((field) => <label key={field}>{field}<input value={variant[field]} onChange={(event) => updateVariant(index, { [field]: event.target.value } as Partial<VariantRow>)} /></label>)}</div><div className="private-form-row"><label>Stock<input type="number" min="0" value={variant.stock} onChange={(event) => updateVariant(index, { stock: event.target.value })} /></label><label>Pi price<input type="number" min="0" step="any" value={variant.pricePi} onChange={(event) => updateVariant(index, { pricePi: event.target.value })} /></label><label>USDT price<input type="number" min="0" step="any" value={variant.priceUsdt} onChange={(event) => updateVariant(index, { priceUsdt: event.target.value })} /></label><label>Variant image<input type="file" accept="image/*" onChange={(event) => { selectVariantImage(index, event.target.files?.[0]); event.currentTarget.value = ""; }} /></label></div>{variant.image ? <img className="product-variant-image" src={variant.image} alt="" /> : null}</article>)}</div> : null}
      {form.variants.length ? <div className="variant-preview-list"><strong>Product Variations Preview</strong>{form.variants.map((variant, index) => <span key={index}>{variantFields.map((field) => variant[field]).filter(Boolean).join(" / ") || `Variant ${index + 1}`} · Price: {variant.pricePi ? `${variant.pricePi} π` : "Base price"} · Stock: {variant.stock || 0}</span>)}</div> : null}
