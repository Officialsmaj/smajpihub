...
const featureCards = [
  { title: "Take care of your health", text: "Find doctors, pharmacies and health services", slug: "health", image: "https://picsum.photos/seed/smaj-health-mobile/900/620" },
  { title: "Find your next home", text: "Discover houses, apartments and properties", slug: "housing", image: "https://picsum.photos/seed/smaj-housing-mobile/900/620" },
  { title: "Build your skills", text: "Courses, training and certifications", slug: "education", image: "https://picsum.photos/seed/smaj-learning-mobile/900/620" },
  { title: "Discover new products", text: "Shop from sellers and stores", slug: "store", image: "https://picsum.photos/seed/smaj-shopping-mobile/900/620" },
];
const mediaSections = [
  { title: "Watch anytime", slug: "stream", items: ["Movies", "Series", "Videos", "Creators", "Live content"] },
  { title: "Live sports", slug: "sports", items: ["Football", "Scores", "Teams", "Games", "Community"] },
  { title: "Events happening now", slug: "events", items: ["Concerts", "Conferences", "Shows", "Meetups", "Experiences"] },
];

const ServiceStrip = ({ services }: { services: ServiceDefinition[] }) => <div className="mobile-service-groups">{Array.from({ length: Math.ceil(services.length / 3) }, (_, groupIndex) => <div className="mobile-service-group" key={groupIndex}>{services.slice(groupIndex * 3, groupIndex * 3 + 3).map((service) => <Link to={servicePath(service)} className="mobile-service-app" key={service.slug}><ServiceArt index={service.atlasIndex} /><div><strong>{service.name}</strong><span>{service.items.slice(0, 2).join(" • ")}</span></div><ArrowForwardOutlinedIcon /></Link>)}</div>)}</div>;
...
const MobileHome = () => {
  ...
  return <div className="mobile-super-home">
    <section className="mobile-home-hero">
      <div className="mobile-home-hero-copy"><span>SMAJ PI HUB</span><h1>Everything you need.<br />One place.</h1><div className="mobile-hero-icons">{serviceCatalog.slice(0, 4).map((service) => <ServiceArt key={service.slug} index={service.atlasIndex} />)}<b>15+</b></div><Link to="/app/services">Explore <ArrowForwardOutlinedIcon /></Link></div>
    </section>
...
const DesktopHome = () => <div className="desktop-private-home"><section className="ecosystem-hero"><div><p className="private-kicker">SMAJ PI HUB</p><h1>Everything you need.<br />One place.</h1><p>Discover services built for everyday life.</p><Link className="private-primary-button" to="/app/services">Explore Services <ArrowForwardOutlinedIcon /></Link></div><div className="ecosystem-hero-art">{serviceCatalog.slice(0,6).map((service) => <ServiceArt key={service.slug} index={service.atlasIndex} />)}</div></section>
  ...
  <section className="assistant-callout"><AutoAwesomeOutlinedIcon /><div><p className="private-kicker">SMAJ PI HUB AI ASSISTANT</p><h2>Your smart guide inside SMAJ PI HUB</h2><p>Find services, get help, and navigate faster.</p><div><span>"Find a service for me"</span><span>"Help me use SMAJ PI HUB"</span><span>"Show what I can do"</span></div></div><Link className="private-primary-button" to="/assistant">Open AI Assistant</Link></section>
