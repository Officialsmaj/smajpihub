...
const countries = Country.getAllCountries().map((country) => ({ ...country, flag: country.flag || flagFor(country.isoCode) })).sort((a, b) => a.name.localeCompare(b.name));
...
const AmbassadorServicesPanel = () => {
  ...
  const regions = useMemo(() => form.countryCode ? State.getStatesOfCountry(form.countryCode) : [], [form.countryCode]);
    ...
    {mode ? <div className="ambassador-modal-backdrop" role="presentation" onMouseDown={close}>
      <section className="ambassador-modal" role="dialog" aria-modal="true" aria-label={mode === "apply" ? "Ambassador application" : "Find ambassadors"} onMouseDown={(event) => event.stopPropagation()}>
        ...
        {mode === "find" ? <div className="ambassador-directory">
          ...
          {directoryLoading ? <p>Loading ambassadors…</p> : ambassadors.length ? <div className="ambassador-directory-list">{ambassadors.map((ambassador) => <article key={ambassador._id}><span>{ambassador.countryFlag || flagFor(ambassador.countryCode)}</span><div><strong>{ambassador.displayName}</strong><p>{ambassador.regionName}, {ambassador.countryName}</p><small>{ambassador.services.includes("all") ? "All SMAJ services" : ambassador.services.join(" · ")}</small></div><VerifiedUserOutlinedIcon /></article>)}</div> : <div className="ambassador-empty"><GroupsOutlinedIcon /><strong>No approved ambassadors found</strong><p>Try another country or service.</p></div>}
          ...
          <fieldset><legend>Services</legend><div className="ambassador-service-options">{serviceOptions.map((service) => <button type="button" key={service.slug} className={form.services.includes(service.slug) ? "selected" : ""} onClick={() => chooseService(service.slug)}>{service.name}</button>)}</div></fieldset>
