  ...
  router.post("/", async (req: Request, res: Response) => {
    try {
      ...
      if (!fullName || !email || !applicationType || !location || details.length < 20 || !email.includes("@")) {
        return res.status(400).json({ error: "bad_request", message: "Please complete all fields. The details section must be at least 20 characters." });
      }

      if (!req.app.locals.onboardingCollection) {
        return res.status(503).json({ error: "service_unavailable", message: "Application service is not ready. Please try again in a moment." });
      }
      ...
      return res.status(500).json({ error: "submit_failed", message: "Application could not be saved right now. Please try again." });
    }
  });
}
